...
$cari = mysqli_query($koneksi,"select * from cart where id_pelanggan='$ui' and status='cart'");
while ($fetc = mysqli_fetch_array($cari)) {
    $orderid = $fetc['orderid'];
}
$itung = mysqli_query($koneksi,"select count(orderid) as jumlahtrans from cart where id_pelanggan='$ui' and status!='order'");
...
if (isset($_POST["update"])) {
    ...
    if ($q1) {
        echo "berhasil update keranjang
        <meta http-equiv='refresh' content='1; url=../transaksi/keranjang.php/>";
    }else {
        echo "gagal update keranjang
        <meta http-equiv='refresh' content='1; url=../transaksi/keranjang.php/>";
    }
}elseif (isset($_POST["hapus"])) {
    $kode = $_POST['idproduknya'];
	$q2 = mysqli_query($conn, "delete from detailorder where id_produk='$kode' and orderid='$orderid'");
	if($q2){
		echo "Berhasil Hapus";
	} else {
		echo "Gagal Hapus";
		...
		<div class="main-panel">
			<div class="content">
				<div class="page-inner">
					<div class="mt-0 mb-4">
						<h2 class="text-white pb-2">Data Order</h2>
                        <h4>Anda telah melakukan <span><?php echo $itung3 ?> transaksi</span></h4>
					</div>
								...
								<div class="panel panel-default">
									...
									<div class="panel-body">
										<div class="table-responsive">
                                            ...
                                            <?php
                                            $no = 1;
                                            $brg = mysqli_query($koneksi,"SELECT DISTINCT(idcart), c.orderid, tanggal, status from cart c, detailorder d where c.id_pelanggan='$ui' and d.orderid=c.orderid and status!='order' order by tanggal DESC");
                                            while($b=mysqli_fetch_array($brg)){

                                                ?>
                                                <tr class="rem1"><form method="post">
                                                    <td><?php echo $no++ ?></td>
                                                    <td ><a href="order1.php?id=<?php echo $b['orderid']; ?>">
                                                    <?php echo $b['orderid']?></a></td>
                                                    <td ><?php echo $b['tanggal'] ?></td>
                                                    <td >
                                                    Rp<?php 
												$ordid = $b['orderid'];
												$result1 = mysqli_query($koneksi,"SELECT SUM(qty*harga) AS count FROM detailorder d, produk p where d.orderid='$ordid' and p.id_produk=d.id_produk order by d.id_produk ASC");
