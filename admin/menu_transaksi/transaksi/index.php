...
if (isset($_POST['submit'])) {
    ...
    $j = mysqli_query($koneksi, "SELECT * FROM cart c, pelanggan l where c.id_pelanggan=l.id_pelanggan ");

	$brg = mysqli_query($koneksi,"SELECT * from cart c, pelanggan p where c.id_pelanggan=p.id_pelanggan  and status!='selesai' and status!='bayar' and status!='Pembatalan' order by idcart asc");
	...
	<div class="wrapper">
		...
		<div class="main-panel">
			<div class="content">
				<div class="page-inner">
					<div class="mt-0 mb-4">
						...
						<div class="d-sm-flex justify-content-between align-items-center">
                                    ...
                                    <a href="pesananselesai.php" style="margin-bottom:20px"  class="btn btn-info ">Pesanan Selesai</a>
								</div>
									...
									<div class="panel-body">
										<div class="table-responsive">
											<table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                                ...
                                                $no = 1;
												$brg = mysqli_query($koneksi,"SELECT * from cart c, pelanggan p where c.id_pelanggan=p.id_pelanggan  and status!='selesai' and  status!='Pembatalan' order by idcart asc");
												...
												$result1 = mysqli_query($koneksi,"select sum(d.qty*p.harga) AS count from detailorder d, produk p where orderid='$orderid' and p.id_produk=d.id_produk order by d.id_produk ASC");
												...
												if($cekrow > 0){
													...
													if($cekroww > 0){
														...
														if($r['status']=='Pengiriman'){
															echo "Pengiriman Sedang Berlangsung";
														} elseif ($r['status']=='order') {
															echo "Pembayaran Belum Dilakukan";
														}elseif ($r['status']=='TerVerifikasi') {
															echo "Pembayaran Sudah di Verifikasi";
				...
				<div class="page-inner">
					<h4 class="text-white pb-2">Total Transaksi Belum di Proses</h4>
					<ul>
						<?php 


						$brg=mysqli_query($koneksi,"select * from detailorder d, produk p, cart c where c.orderid=d.orderid and d.id_produk=p.id_produk and status!='selesai' and status!='Pembatalan' and status!='Pengiriman' and status!='TerVerifikasi' order by c.idcart ASC ");
						
						//$brg=mysqli_query($koneksi,"SELECT * from detailorder d, produk p where orderid and d.id_produk=p.id_produk order by d.id_produk ASC");

						//$brg=mysqli_query($koneksi,"select * from cart c, detailorder d, produk p where d.orderid=c.orderid and d.id_produk=p.id_produk and status!=order order by d.idproduk asc");
